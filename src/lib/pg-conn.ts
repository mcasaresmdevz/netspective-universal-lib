  ...
  if (configTextSupplied) {
    if (configTextSupplied.match(pgConnUrlPattern)) {
      ...
      try {
        ...
        return { isConfigured: false, configTextSupplied, configTextParseError: err as Error };
      }
    }
  } else {
...
export const memoizableSqlResults = (options: { readonly isDbAvailable: () => boolean }) => {
...
export const prepareConnsFactory = (configure: () => DatabasesConfiguration) => {
  ...
  return () => {
    ...
    factory = {
      dbConfig,
      connSqlInstances,
      conn: async (connID: string = PostgreSqlDefaultConnID): Promise<SqlConnState> => {
        const connURL = dbConfig?.isConfigured ? dbConfig.connURLs?.URLs[connID] : undefined;
        if (!connURL) {
          const SQL = postgres();
          return {
            connID,
          ...
          try {
            ...
            if (testResult && testResult.length == 1) {
              ...
              connState = {
                ...
                state: 'valid',
                connection,
                connUrlInfo: `postgres://${connection.user}@${connection.host.join(
                  ','
                )}:${connection.host.join(',')}/${connection.database}`,
                SQL,
...
export const dbConnsFactory = (() => {
  const cf = prepareConnsFactory(() => {
    const result = dbConfigureFromText(
      () => process.env.SERVICE_DB_CONN_URLS ?? import.meta.env.SERVICE_DB_CONN_URLS
    );
