...
export interface GitWorkTreeAssetResolver {
  (candidate: GitEntry, gitBranchOrTag: GitBranchOrTag, paths: GitPathsSupplier):
    | GitWorkTreeAsset
    | undefined;
}
...
export function workspaceEditorResolver(
  ...
  switch (type) {
    ...
    case 'vscode-wsl':
      return vscodeWslRemoteEditorResolver(options?.vscodeWslRemoteDistro?.() ?? 'Debian');
...
export function typicalGitWorkTreeAssetUrlResolvers<Identity extends string>(
  ...
  const gitAssetUrlResolvers = new Map<string, GitWorkTreeAssetUrlResolver<Identity>>();
...
export function gitLabResolvers(gitLabRemoteUrlPrefix: string, remoteServerHumanName: string) {
  ...
  const mGitResolvers: ManagedGitResolvers<string> = {
    ...typicalGitWorkTreeAssetUrlResolvers<string>(assetUrlResolver, gitWorkTreeAssetVsCodeURL),
