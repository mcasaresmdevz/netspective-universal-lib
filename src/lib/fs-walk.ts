...
export async function walkFsEntries(startDir: string, options?: {
  ...
  for (const entry of entries) {
    ...
    if (stat.isDirectory()) {
      const instruction = onDirectory ? await (onDirectory?.({parentRelPath, absPath, relPath, stat, level}) ?? "enter") : "enter";
      switch(instruction) {
        case "enter":
