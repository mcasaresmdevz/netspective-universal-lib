 ...
 */
export const qualifiedPathPreparer = <Terminal, Intermediary>(pathSep = "/") => {
  return (unit: string, parent?: PathTreeNode<Terminal, Intermediary>) =>
    parent?.qualifiedPath
      ? (parent?.qualifiedPath + pathSep + unit)
      : unit;
}
...
export function populatePathTree<Terminal, Intermediary>(
  ...
  intermediarySupplier: (ptn: Omit<PathTreeNode<Terminal, Intermediary>, "intermediary">, units: string[]) => PathTreeNode<Terminal, Intermediary>,
  options?: {
  ...
  const nodeExists = options?.nodeExists ?? ((prospect, siblings) => siblings.find((cn) => cn.unit == prospect));
  const nodeQualifiedPath = options?.qualifiedPathPreparer ?? qualifiedPathPreparer();
...
export function pathTreeIndex<Terminal, Intermediary, Key = string>(
  ...
  return result.reduce((index, node) => {
    index.set(keySupplier ? keySupplier(node) : node.qualifiedPath as Key, node);
    return index;
...
export function pathTree<Terminal, Intermediary>(
  ...
  const tree: PathTree<Terminal, Intermediary> = options?.tree ?? { children: [] }
