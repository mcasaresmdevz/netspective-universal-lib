...
export function singleton<A extends unknown[], V>(fn: (...args: A) => Promise<V>): typeof fn {
 ...
 */
// DEV-NOTE from original author: we cannot use async/await because of the ES6 target
export function singletonSync<A extends unknown[], V>(fn: (...args: A) => V): typeof fn {
    let cached: V | undefined = undefined;
    return (...args: A) => {
        if(!cached) {
            cached = fn(...args);
 ...
 */
// DEV-NOTE from original author: we cannot use async/await because of the ES6 target
export function memoize<A extends unknown[], K, V>(fn: (...args: A) => Promise<V>, store?: MemoizeStoreStrategy<K, V>): typeof fn {
    const { get, set, toKey } = store ?? memMemoizeStoreStrategy<V>() as MemoizeStoreStrategy<unknown, V>;
 ...
 */
export function memIdMemoizeStoreStrategy<V>(keySupplier: (args: unknown[]) => string, cache: Map<string, V> = new Map()): MemoizeStoreStrategy<string, V> {
    return {
 ...
 * @returns a MemoizeStoreStrategy instance
 */
export function memMemoizeStoreStrategy<V>(cache: Map<string, V> = new Map()): MemoizeStoreStrategy<string, V> {
    return memIdMemoizeStoreStrategy<V>((args) => JSON.stringify(args), cache);
}

export function ensureFsPathExists(dir: string, cb: (err: NodeJS.ErrnoException | null) => void) {
 ...
 */
export function fsTextMemoizeStoreStrategy(options: FileSysMemoizeStoreStrategyFactory<string>): MemoizeStoreStrategy<string, string> {
 ...
 *          Invalid characters will be replaced with dash `-`.
 */
export function fsJsonMemoizeStoreStrategy<V>(options: FileSysMemoizeStoreStrategyFactory<V>): MemoizeStoreStrategy<string, V> {
    const fnSupplier = options.fileNameSupplier;
    const ensureFSP = options?.ensureFsPathExists ?? ensureFsPathExists;
    const acceptPersistedFile = options?.acceptPersistedFile;
    return {
